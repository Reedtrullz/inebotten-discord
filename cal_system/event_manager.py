...
class EventManager:
    ...
    def _load_events(self):
        ...
        if self.storage_path.exists():
            try:
                ...
                print(f"[CALENDAR] Event load error: {e}")
                return {}
        return {}
            ...
            if self.gcal_enabled:
                try:
                    ...
                    if gcal_result:
                        ...
                        print(
                            f"[EVENT] Synced to Google Calendar: {gcal_result.get('htmlLink')}"
                        )
                except Exception as e:
    ...
    def get_upcoming_events(self, guild_id, days=30, include_gcal=True):
        ...
        for event in local_events:
            try:
                ...
                if (
                    event.get("gcal_event_id")
                    and event["gcal_event_id"] in gcal_event_ids
                ):
                    continue
                ...
                print(f"[CALENDAR] Event loop error: {e}")
                continue
    ...
    def get_todays_events(self, guild_id):
        ...
        if str(guild_id) in self.events:
            todays = [
                e
                for e in self.events[str(guild_id)]
    ...
    def format_event_list(self, events, title="Kommende arrangementer"):
        ...
        for i, event in enumerate(events[:10], 1):  # Show max 10, numbered
            ...
            gcal_indicator = (
                "📅"
                if event.get("gcal_event_id") or event.get("is_gcal_event")
                else "📌"
            )
            ...
            if event.get("recurrence"):
                ...
                if event.get("recurrence_day"):
                    ...
                    recurrence_str = (
                        f" 🔄 {recurrence_labels.get(event['recurrence'], '')}"
                    )

            lines.append(
                f"{gcal_indicator} **{event['title']}** - {event['date']}{time_str}{recurrence_str}"
            )
    ...
    def format_single_event(self, event):
        ...
        if event.get("recurrence"):
            ...
            if event.get("recurrence_day"):
                lines.append(
                    f"🔄 Gjentas hver {event['recurrence_day']} ({recurrence_labels.get(event['recurrence'], event['recurrence'])})"
                )
            else:
                lines.append(
                    f"🔄 Gjentas {recurrence_labels.get(event['recurrence'], event['recurrence'])}"
                )
...
def parse_event_command(message_content):
    ...
    result = {"title": title, "date": date_str, "time": time_str}
