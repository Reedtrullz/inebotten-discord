...
class ReminderManager:
    ...
    def add_reminder(self, guild_id, user_id, username, text, due_date=None,
            ...
            for i, r in enumerate(active[:8], 1):  # Show max 8
                ...
                if r.get('recurrence'):
                    ...
                    if r.get('recurrence_day'):
                        ...
                        recurrence_str = f" 🔄 {recurrence_labels.get(r['recurrence'], '')}"
    ...
    def delete_old_completed(self, guild_id, days=7):
        ...
        self.reminders[guild_key] = [
            r for r in self.reminders[guild_key]
            if not r['completed'] or 
            (r.get('completed_at') and datetime.fromisoformat(r['completed_at']) >= cutoff)
        ]
...
def parse_reminder_command(message_content):
    ...
    reminder_keywords = ['påminnelse', 'husk å', 'husk at', 'reminder', 'gjøremål', 'todo']
    ...
    if any(word in content_lower for word in ['påminnelser', 'gjøremål', 'reminders', 'todos', 'huskeliste']):
        return {'action': 'list'}
