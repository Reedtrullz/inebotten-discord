...
class MessageMonitor:
    ...
    def __init__(self, client, hermes_connector, rate_limiter, response_generator, bot_name="inebotten"):
        self.client = client
    ...
    async def handle_message(self, message):
        ...
        print(f"[MONITOR] Mention detected from {message.author.name}: {message.content[:50]}...")
        ...
        if any(word in content_lower for word in ['kalender', 'calendar', 'arrangementer', 'events', 'kommende', 'planlagt', 'påminnelser', 'huskeliste']):
        ...
        if any(word in content_lower for word in ['ferdig', 'done', 'complete', 'fullført']):
        ...
        if any(phrase in content_lower for phrase in ['dagens ord', 'word of the day', 'lære meg et ord']):
        ...
        if any(phrase in content_lower for phrase in ['skoleferie', 'skoleferier', 'vinterferie', 'påskeferie']):
        ...
        if any(phrase in content_lower for phrase in ['daglig oppsummering', 'daily digest', 'oppsummering', 'summary']):
            print(f"[MONITOR] Matched: daily digest")
            await self._handle_daily_digest(message)
            return
        
        # Check for help command
        if any(word in content_lower for word in ['hjelp', 'help', 'kommandoer', 'commands']):
    ...
    async def _handle_calendar_item(self, message, item_data):
        ...
        try:
            ...
            if item:
                ...
                response_text = "❌ Beklager, jeg klarte ikke å legge til i kalenderen. Prøv igjen!"
    ...
    async def _handle_complete_item(self, message):
        ...
        try:
            ...
            if num_match:
                item_num = int(num_match.group(1))
                success, title, next_date = self.calendar.complete_item(guild_id, item_num)
                ...
                calendar_text = self.calendar.format_list(guild_id)
                if calendar_text:
                    response_text = f"📝 Hvilket vil du markere som fullført?\n\n{calendar_text}"
                else:
                    response_text = "📭 Kalenderen er tom."
    ...
    async def _handle_edit_event(self, message):
        ...
        try:
            ...
            if not items:
                ...
                response_text += "For å endre et arrangement, slett det først og lag et nytt:\n\n"
                for i, item in enumerate(items[:5], 1):
                    response_text += f"{i}. **{item['title']}** - {item['date']}\n"
                response_text += "\nBruk: `@inebotten slett [nummer]`\n"
                response_text += "Deretter: `@inebotten [nytt arrangement] [dato] [tid]`"
    ...
    async def _handle_aurora_command(self, message):
        """
        Handle aurora forecast command
        """
        try:
            ...
            if forecast:
                response_text = self.conv_gen.format_aurora_forecast(forecast)
            else:
                response_text = "Kunne ikke hente nordlysvarsel akkurat nå. Prøv igjen senere! 🌌"
    ...
    async def _handle_school_holidays_command(self, message):
        """
        Handle school holidays command
        """
        try:
            from features.school_holidays import format_holidays_list, get_fylke_from_location
            
            # Try to detect fylke from message content
            content_lower = message.content.lower()
            ...
            # Add help text if no specific fylke detected
            if not fylke:
                if lang == 'no':
            ...
            print(f"[MONITOR] Error handling school holidays command: {e}")
    ...
    async def _handle_vote(self, message, vote_num):
        ...
        try:
            ...
            if not active_polls:
                ...
                poll = active_polls[-1]
                success, msg = self.poll.vote(guild_id, poll['id'], vote_num, message.author.id, message.author.name)
    ...
    async def _handle_compliment_command(self, message, compliment_cmd):
        ...
        try:
            ...
            if compliment_cmd['action'] == 'compliment':
                ...
                response_text = self.compliments.format_compliment(text, compliment_cmd.get('user'), lang)
            else:  # roast
                text = self.compliments.get_roast(lang)
                response_text = self.compliments.format_roast(text, compliment_cmd.get('user'), lang)
    ...
    async def _handle_daily_digest(self, message):
        ...
        try:
            ...
            print(f"[MONITOR] Error handling daily digest: {e}")
    ...
    async def _handle_help_command(self, message):
        ...
        try:
            ...
            # Build help text from localization
            lines = [
            ...
            print(f"[MONITOR] Error handling help command: {e}")
    ...
    async def _handle_edit_event(self, message):
        ...
        response_text = "✏️ Redigering kommer snart!\n\nI mellomtiden kan du slette og legge til på nytt."
        ...
        if not wants_dashboard:
            try:
                if self.hermes:
                    ...
                    conversation_context = self.conversation.get_context(guild_id, limit=5)
        ...
        if not response_text:
            if wants_dashboard:
                ...
                if user.get('username') or user.get('last_interaction'):
                    greeting = self.user_memory.get_personalized_greeting(
                        message.author.id,
                        message.author.name
                    ) + "\n\n"
...
class SelfbotClient(discord.Client):
    ...
    def __init__(self, config, auth_handler, rate_limiter, hermes_connector, response_generator):
    ...
    async def on_ready(self):
        ...
        print(f"[BOT] Rate limit: {self.config.MAX_MSGS_PER_SECOND}/sec, {self.config.DAILY_QUOTA}/day")
