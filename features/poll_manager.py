...
class PollManager:
    ...
    def create_poll(self, guild_id, question, options, created_by):
        ...
        poll = {
            ...
            'question': question,
            'options': [{'text': opt, 'votes': [], 'emoji': self.emojis[i]} for i, opt in enumerate(options[:10])],
...
def parse_poll_command(message_content):
    ...
    # Detect language
    lang = 'no' if any(word in content_lower for word in ['avstemning', 'stemme', 'eller']) else 'en'
    ...
    if match:
        ...
        options = [opt.strip() for opt in re.split(r'\s+(?:eller|or)\s+', rest, flags=re.IGNORECASE) if opt.strip()]
        if len(options) >= 2:
            return {'question': question, 'options': options, 'lang': lang}
