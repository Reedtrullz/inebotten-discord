...
class QuoteManager:
    ...
    def get_quote_by_author(self, guild_id, author):
        ...
        if guild_key in self.quotes:
            candidates.extend([q for q in self.quotes[guild_key] if author.lower() in q['author'].lower()])
        
        # Search other guilds too
        for gid, quotes in self.quotes.items():
            if gid != guild_key:
                candidates.extend([q for q in quotes if author.lower() in q['author'].lower()])
    ...
    def format_quote(self, quote, lang='no'):
        ...
        header = '💬 **Sitat fra arkivet**' if lang == 'no' else '💬 **Quote from archive**'
...
def parse_quote_command(message_content):
    ...
    # Detect language
    lang = 'no' if any(word in content_lower for word in ['husk', 'lagre', 'gullkorn', 'sitat']) else 'en'
    ...
    save_phrases_en = ['remember this', 'save this', 'this must be remembered', 'quote this']
