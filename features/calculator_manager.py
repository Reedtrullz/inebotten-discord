...
class CalculatorManager:
    ...
    def parse_command(self, message_content):
        ...
        currency_pattern = r'(?:(?:konverter|convert)\s+)?(\d+(?:\.\d+)?)\s*(\w+)\s+(?:til|to)\s+(\w+)'
    ...
    def _convert_currency(self, cmd, lang):
        ...
        elif to_curr in self.exchange_rates and from_curr in self.exchange_rates[to_curr]:
            rate = 1 / self.exchange_rates[to_curr][from_curr]
        else:
            # Default fallback
            rate = random.uniform(0.5, 15)
