...
@pytest.mark.asyncio
async def test_send_response_drops_when_wait_refuses_send():
    rate_limiter = SimpleNamespace(
        can_send=Mock(return_value=(True, None)),
        wait_if_needed=AsyncMock(return_value=False),
        record_sent=Mock(),
        record_failure=Mock(),
        record_dropped=Mock(),
    )
...
@pytest.mark.asyncio
async def test_send_response_accepts_legacy_sync_wait_mock():
    rate_limiter = SimpleNamespace(
        can_send=Mock(return_value=(True, None)),
        wait_if_needed=Mock(return_value=True),
        record_sent=Mock(),
        record_failure=Mock(),
    )
    monitor = SimpleNamespace(rate_limiter=rate_limiter, loc=object(), client=object(), response_count=0)
